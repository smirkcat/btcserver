...
func recentTransactions(addr string, count, skip int) ([]wallet.Transactions, error) {
	...
	translog, err := dbengine.GetTransactions(addr, count, skip)
	if err != nil {
		return trans, err
	}
	lens := len(translog)
	...
	for i := 0; i < lens; i++ {
		...
		trans[i].Address = translog[i].Toaddr
		...
		trans[i].Confirmations = blockHeightTop - translog[i].Blockindex
		trans[i].Time = translog[i].Timestamp
		trans[i].BlockIndex = translog[i].Blockindex
		trans[i].Account = "go-omni-walletrpc"
	}
	return trans, nil
}

func sendaddr(addr string, amount json.Number) (string, error) {
	return node.OmniSend(mainAddr, addr, propertyid, amount.String())
}

func collectTransactions(sTime, eTime int64) ([]wallet.SummaryData, error) {
	var collects []wallet.SummaryData
